const CreateJobs = {
    run: function(room) {
        ...
        // new jobs
        const activeSources                 = room.find(FIND_SOURCES_ACTIVE).map(function (p) { return p.id; });

        const droppedResources              = room.find(FIND_DROPPED_RESOURCES, {filter: (drop) => {return (drop.amount > 50);}}).map(function (p) { return p.id; });
        const spawnsAndExtensionsNeedEnergy = room.find(FIND_MY_STRUCTURES, {filter: (structure) => {return ((structure.structureType == STRUCTURE_SPAWN || structure.structureType == STRUCTURE_EXTENSION) && structure.energy < structure.energyCapacity);}}).map(function (p) { return p.id; });
        const towersNeedEnergy              = room.find(FIND_MY_STRUCTURES, {filter: (tower) => {return (tower.structureType == STRUCTURE_TOWER && tower.energy < tower.energyCapacity);}}).map(function (p) { return p.id; });
        const fullLinks                     = room.find(FIND_MY_STRUCTURES, {filter: (link) => {return (link.structureType == STRUCTURE_LINK && link.energy < link.energyCapacity);}}).map(function (p) { return p.id; });
        const fullContainers                = room.find(FIND_STRUCTURES, {filter: (container) => {return (container.structureType == STRUCTURE_CONTAINER && _.sum(container.store) > 0);}}).map(function (p) { return p.id; });

        const ownedControllers              = room.find(FIND_STRUCTURES, {filter: (controller) => {return (controller.structureType == STRUCTURE_CONTROLLER);}}).map(function (p) { return p.id; });

        const damagedStructures             = room.find(FIND_STRUCTURES, {filter: (structure) => {return (structure.hits < structure.hitsMax);}}).map(function (p) { return p.id; });
        const constructions                 = room.find(FIND_CONSTRUCTION_SITES);

        const activeExtractors              = room.find(FIND_MY_STRUCTURES, {filter: (extractor) => {return (extractor.structureType == STRUCTURE_EXTRACTOR && extractor.isActive());}}).map(function (p) { return p.id; });

        const hostileCreeps                 = room.find(FIND_HOSTILE_CREEPS, {filter: (creep) => {return (creep.owner.username != "Invader");}}).map(function (p) { return p.id; });
        ...
        function CheckForNewJobs(newJobIDsArray, closedJobsArray, openJobsArray){
            if(newJobIDsArray === undefined){
                newJobIDsArray = [];
            }
            if(closedJobsArray === undefined){
                closedJobsArray = [];
            }
            if(openJobsArray === undefined){
                openJobsArray = [];
            }
            let newOpenJobsArray = [];
            // loop through all new jobs
            for(const newJobIDCount in newJobIDsArray){
                const newJobID = newJobIDsArray[newJobIDCount]; // const newJob = Game.getObjectById(newJobID);
                let isClosedJobFound = false;
                let foundExistingOpenJob = undefined;

                for(const closedJobsCount in closedJobsArray){
                    const closedJob = closedJobsArray[closedJobsCount];
                ...
                if(!isClosedJobFound){
                    for(const openJobsCount in openJobsArray){
                        const openJob = openJobsArray[openJobsCount];
                ...
                if(foundExistingOpenJob !== undefined){
                    newOpenJobsArray.push(foundExistingOpenJob);
                }else if(!isClosedJobFound){
                    let newJobOBJ = new Object();
                    newJobOBJ.id = newJobID;
                    newOpenJobsArray.push(newJobOBJ);
                    newJobsCounter++;
                }
            }
            return newOpenJobsArray;
        }
    }
};
