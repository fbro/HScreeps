        ...
        function ActivateDefensiveMeasures(gameRoom, hostileTargets, towers) {
            ...
            for (const hostileTargetCount in hostileTargets) {
                ...
                if (towers.length === 0 || hostileTarget.owner.username !== 'Invader' && (hostileTarget.getActiveBodyparts(RANGED_ATTACK) || hostileTarget.getActiveBodyparts(ATTACK) || hostileTarget.getActiveBodyparts(HEAL))) {
                    ...
                    if (isBoosted) {
                        Util.InfoLog('Towers', 'ActivateSafemode', gameRoom.name + ' attacked from ' + hostileTarget.owner.username + ' towers ' + towers.length + ' time ' + Game.time);
                        if (!gameRoom.controller.safeMode && !gameRoom.controller.safeModeCooldown && gameRoom.controller.safeModeAvailable > 0) {
                            ...
                            Util.InfoLog('Towers', 'ActivateSafemode', gameRoom.name + ' ' + result + ' attacked from ' + hostileTarget.owner.username + ' towers ' + towers.length + ' time ' + Game.time);
                            Game.notify('safemode have been activated for room ' + gameRoom.name + ' activateSafeMode result ' + result + ' shard ' + Game.shard + ' attacked from ' + hostileTarget.owner.username + ' time ' + Game.time, 0);
                        } else {
                            Util.InfoLog('Towers', 'ActivateSafemode', gameRoom.name + ' cannot activate safemode! controller.safeMode ' + gameRoom.controller.safeMode + ' controller.safeModeCooldown ' + gameRoom.controller.safeModeCooldown + ' controller.safeModeAvailable ' + gameRoom.controller.safeModeAvailable);
                        }
                    }
