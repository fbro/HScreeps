        ...
        */
        ...
        while (!isAllAssigned) {
                    ...
                    for (let e = 0; e < idleCreeps.length; e++) { // loop through all idle creeps
                        ...
                        if (weight > 0) { // is applicable
                            if (weight < bestWeight) { // best creep for the job (for now) is found
                                ...
                                if (openJob.pos.roomName === creep.pos.roomName) { // in same room
                                    positionWeight += -JOB_WEIGHT_MOD;
                                    positionWeight += Math.sqrt(Math.pow(openJob.pos.x - creep.pos.x, 2) + Math.pow(openJob.pos.y - creep.pos.y, 2)) * JOB_WEIGHT_MULTIPLIER_INTER_ROOM; // in room range
                                } else { // Get the linear distance (in rooms) between two rooms
                                    positionWeight = JOB_WEIGHT_MOD * Game.map.getRoomLinearDistance(openJob.pos.roomName, creep.pos.roomName);
                                }
            ...
            if (bestCreep) { // best creep for the job is now assigned
                ...
                let RCL = 0;
                const jobRoom = Game.rooms[bestOpenJob.pos.room];
                if(jobRoom){
                    RCL = jobRoom.controller.level;
                }
            ...
            for (let i = 0; i < Memory.openJobs.length; i++) { // loop through all open jobs
                ...
                if(!AtCreepRoof(openJob.name, openJob, false)){
                    for (let e = 0; e < availableSpawns.length; e++) {
                        ...
                        if (spawn.pos.roomName === openJob.pos.roomName) { // same room
                            weight += -RANGE_WEIGHT_MOD;
                            weight += Math.sqrt(Math.pow(openJob.pos.x - spawn.pos.x, 2) + Math.pow(openJob.pos.y - spawn.pos.y, 2)) * RANGE_WEIGHT_MULTIPLIER_INTER_ROOM; // in room range
                        } else { // Get the linear distance (in rooms) between two rooms
                            weight += RANGE_WEIGHT_MOD * Game.map.getRoomLinearDistance(openJob.pos.roomName, spawn.pos.roomName);
                        }
         ...
         * @return {boolean}
         */
        function AtCreepRoof(jobName, openJob, isAssigningIdleCreep){
            let creepInitials = "X";
            ...
            for (const creepName in Game.creeps) {
                ...
                if(creep.name.startsWith(creepInitials)){
                    if(creep.memory.jobName === "idle"){
                        ...
                        if(job && openJob) {
                            if (job.pos.roomName === openJob.pos.roomName) {
                                creepCount++;
                            }
                        }else{
